...
class Mesh3D(object):
    ...
    @property
    def vertices(self):
        """:obj:`numpy.ndarray` of float : A #verts by 3 array, where each row
        contains an ordered [x,y,z] set that describes one vertex."""
    ...
    @property
    def triangles(self):
        """:obj:`numpy.ndarray` of int : A #tris by 3 array, where each row
        contains indices of vertices in the `vertices` array that are part of
        the triangle."""
    ...
    def normals(self):
        ...
        A #normals by 3 array, where each row contains a normalized vector. This list should contain one norm per
        vertex.
        """
    ...
    def normalize_vertices(self):
        ...
        Transforms the vertices and normals of the mesh such that the origin of the resulting mesh's coordinate frame
        is at the center of the bounding box and the principal axes (as determined from PCA) are aligned with the
        vertical Z, Y, and X axes in that order.
        """
    ...
    def _compute_com_uniform(self):
        ...
        """
        """total_volume = 0 weighted_point_sum = np.zeros([1, 3]) for tri in
        self.triangles_: volume = self._signed_volume_of_tri(tri)

        center = self._center_of_tri(tri)     weighted_point_sum = weighted_point_sum + volume * center total_volume =
        total_volume + volume center_of_mass = weighted_point_sum / total_volume return center_of_mass[0]
        """

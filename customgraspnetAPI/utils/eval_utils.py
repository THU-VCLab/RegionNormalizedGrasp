...
def eval_grasp(grasp_group,
    ...
    for i, model in enumerate(models):
        ...
        seg_mask.append(seg)
        # get sorted target id
        if obj_list[i] + 1 == target:
            sorted_target = i
    ...
    if target is None:
        ...
        grasp_i = grasp_group[model_to_grasp == sorted_target]
        grasp_i.sort_by_score()
        pre_grasp_list = [grasp_i[:10].grasp_group_array]
        all_grasp_list = np.array(grasp_i[:10].grasp_group_array)
    print('\nvalid grasps ==', len(all_grasp_list))
    remain_mask = np.argsort(all_grasp_list[:, 0])[::-1]
    if len(remain_mask) == 0:
        min_score = 0
    else:
        min_score = all_grasp_list[remain_mask[min(TOP_K - 1,
                                                   len(remain_mask) - 1)], 0]
    ...
    if target is None:
        ...
        for i in range(num_models):
            if i == sorted_target:
                remain_mask_i = pre_grasp_list[0][:, 0] >= min_score
                grasp_list.append(pre_grasp_list[0][remain_mask_i])
            else:
                grasp_list.append([])

    ...
    **Input:**
    ...
    def eval_scene(self,
        ...
        **Output:**
        ...
        model_list, dexmodel_list, obj_list = self.get_scene_models(scene_id,
                                                                    ann_id=0)
        ...
        for ann_id in range(256):
            ...
            # eval different scale results
            if scale is not None:
                if scale == 'small':
                    width_mask = (gg_array[:, 1] < 0.04)
                elif scale == 'medium':
                    width_mask = (gg_array[:, 1] >= 0.04) & (gg_array[:, 1]
                                                             < 0.07)
                elif scale == 'large':
                    width_mask = (gg_array[:, 1] >= 0.07)
                else:
                    raise ValueError
                grasp_group.grasp_group_array = gg_array[width_mask]
            else:
                grasp_group.grasp_group_array = gg_array

            if target_oriented:
                target_ids = np.load(f'target_ids_{self.camera}.npy')
                cur_target = target_ids[scene_id, ann_id]
            else:
                cur_target = None

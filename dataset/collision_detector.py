...
class ModelFreeCollisionDetector():
    ...
    def detect(self, grasp_group, approach_dist=0.05, dtype=torch.float16):
        T = torch.from_numpy(grasp_group.translations).to(dtype=dtype,
                                                          device='cuda')
        ...
        depths = torch.full((grasp_group.size, 1),
                            ...
                            dtype=dtype,
                            device='cuda')
        widths = torch.from_numpy(grasp_group.widths[:,
                                                     None]).to(dtype=dtype,
                                                               device='cuda')
        widths += self.delta_width
        points = torch.from_numpy(self.scene_points[None,
                                                    ...]).to(dtype=dtype,
                                                             device='cuda')

...
class Grasp():
    ...
    def to_open3d_geometry(self, color=None, scale=1.0):
        # default gripper setting
        finger_width = 0.004 * scale
        tail_length = 0.04
        depth_base = 0.02
        self.height = 0.002 * scale
...
class GraspGroup():
    ...
    def to_open3d_geometry_list(self, color=None, scale=1.0):
        gripper_list = []
        for i in range(self.size):
    ...
    def nms(self, dis=0.03, angle=60 / 180 * np.pi):
        # use graspnetAPI to nms
        gg = GraspNetGraspGroup(np.zeros((len(self), 17)))
        gg.scores = self.scores
        gg.widths = self.widths
        gg.heights = self.heights
        gg.depths = self.depths
        gg.rotation_matrices = self.rotations
        gg.translations = self.translations
        gg.object_ids = np.full((len(self), ), -1)
        gg = gg.nms(dis, angle)
...
class RectGraspGroup():
    ...
    def __getitem__(self, index):
        # will generate a new Grasp or GraspGroup
        if type(index) == int:
            rect_gg = RectGrasp(center=self.centers[index],
                                height=self.heights[index],
                                width=self.widths[index],
                                depth=self.depths[index],
                                score=self.scores[index],
                                ...
                                object_id=self.object_ids[index])
            if hasattr(self, 'actual_depths'):
                rect_gg.actual_depths = self.actual_depths[index]
            return rect_gg
        elif type(index) in [slice, list, np.ndarray]:
            rect_gg = RectGraspGroup(centers=self.centers[index],
                                     heights=self.heights[index],
                                     widths=self.widths[index],
                                     depths=self.depths[index],
                                     scores=self.scores[index],
    ...
    def load_from_dict(self, grasp_dict, min_width=0, min_score=0.8):
        ...
        # -pi / 2 ~ pi / 2, symmetry
        theta_mask = np.logical_or(self.thetas > np.pi / 2, self.thetas
                                   < -np.pi / 2)
    ...
    def get_gt_6d_width(self, actual_depths):
        """_summary_

        Args:
            actual_depths (_type_): in meters

...
class GraspDataset:
    ...
    def draw_gaussian(self, rect_points, min_dis=8):
        ...
        for points in rect_points:
            ...
            if (center_dis >= min_dis).all():
                ...
                if self.sigma == -1:
                    # rectangle
                    loc_map = draw_rectangle(loc_map, points)
                else:
                    #TODO: ellipse gaussian
                    loc_map = draw_umich_gaussian(loc_map, cur_center,
                                                  self.sigma)
        return loc_map
    ...
    def get_anchor_map(self,
                       ...
                       anchor_z=25.0,
                       use_rect=False):
        """Obtain the training data for Multi-Grasp Generator.

        :param rect_points: rect_points
        :param rect_gg: rect_gg
        :param ratio: Down-sample ratio
        :param anchor_k: Anchor number
        :param anchor_w: Anchor width
        :param anchor_z: Anchor z
        ...
        for points, rect_g in zip(rect_points, rect_gg):
            if use_rect:
                if ratio != 1:
                    raise RuntimeError(
                        'When use rect attri, ratio should be 1!')
                # get rect area
                rr, cc = polygon(points[:, 0], points[:, 1])
                mask = np.logical_and(rr < x_size, cc < y_size)
                ...
                # set, not add
                g_pos = int(g_pos)
                cls_mask_map[g_pos, rr, cc] = 1
                # set, not add
                theta_offset_map[g_pos, rr, cc] = theta_offset
                width_offset_map[g_pos, rr, cc] = width_offset
                if depth_offset is not None:
                    depth_offset_map[g_pos, rr, cc] = depth_offset
            else:
                # get center and width from rect_points (maybe rotate or zoom)
                center = points.mean(0).astype(np.int32)
                u, v = center // ratio
                ...
                elif cls_mask_map[g_pos, u,
                                  v] > 1 and depth_offset_map[g_pos, u, v] > 0:
                    # maintain average if current depth is None
                    depth_offset_map[g_pos, u,
                                     v] *= cls_mask_map[g_pos, u, v] / (
                                         cls_mask_map[g_pos, u, v] - 1)
        if not use_rect:
            # average for offset
            count_map = cls_mask_map + (cls_mask_map == 0)
            theta_offset_map = theta_offset_map / count_map
            width_offset_map = width_offset_map / count_map
    ...
    def __getitem__(self, idx):
        ...
        # plt.subplot(231)
        # plt.imshow(norm_depth.T)
        # plt.subplot(232)
        # plt.imshow(norm_rgb.transpose(2, 1, 0))
        # plt.subplot(233)
        # plt.imshow(loc_map.T, cmap='jet')
        # plt.subplot(234)
        # plt.imshow(cls_mask.mean(0).T)
        # plt.subplot(235)
        # plt.imshow(theta_offset.mean(0).T)
        # plt.subplot(236)
        # plt.imshow(depth_offset.mean(0).T)
        # plt.show()

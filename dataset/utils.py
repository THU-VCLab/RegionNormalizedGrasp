...
def square_distance(points1: torch.Tensor,
    ...
    p1, p2 = points1[:, :3].float(), points2[:, :3].float()
    # calculate dis in shape: (grasp_cnt, label_cnt)
    distance = -2 * torch.matmul(p1, p2.transpose(1, 0))
    N, M = distance.shape
    distance += p1.square().sum(1).view(N, 1)
    distance += p2.square().sum(1).view(1, M)
    return distance
...
def qmul(q0: torch.Tensor, q1: torch.Tensor) -> torch.Tensor:
    ...
    w1, x1, y1, z1 = q1.T
    return torch.stack([
        -x0 * x1 - y0 * y1 - z0 * z1 + w0 * w1, x0 * w1 + y0 * z1 - z0 * y1 +
        w0 * x1, -x0 * z1 + y0 * w1 + z0 * x1 + w0 * y1,
        x0 * y1 - y0 * x1 + z0 * w1 + w0 * z1
    ], 1)


def euler_angles_to_quaternion(euler: torch.Tensor):
    N = euler.shape[0]
    qx = torch.stack([
        torch.cos(euler[:, 0] / 2),
        torch.sin(euler[:, 0] / 2),
        torch.zeros((N, ), device=euler.device),
        torch.zeros((N, ), device=euler.device)
    ], 1)
    qy = torch.stack([
        torch.cos(euler[:, 1] / 2),
...
def rotation_distance(a: torch.Tensor,
    ...
    q_a = euler_angles_to_quaternion(a)
    q_b = euler_angles_to_quaternion(b)
    if symmetry:
        ...
        q_a_r = euler_angles_to_quaternion(a_r)
...
def fast_sample(total_size, sample_num):
    probabilities = np.full((total_size, ), 1.0 / total_size)
    ...
    return np.argpartition(shifted_probabilities, sample_num)[:sample_num]
...
class PointCloudHelper:
    ...
    def to_scene_points(self,
        ...
        points = -torch.ones((batch_size, self.all_points_num, feature_len),
                             dtype=torch.float32).cuda()
        if with_all:
            points_all = -torch.ones(
                (batch_size, rgbs.shape[2], rgbs.shape[3], 3),
                dtype=torch.float32).cuda()
        # cal z
        idxs = []
        masks = (depths > 0)
        ...
        for i in range(batch_size):
            ...
            cur_points = torch.stack([cur_xs[i], cur_ys[i], cur_zs[i]],
                                     axis=-1)
            if with_all:
                points_all[i] = cur_points
            # remove zero depth
            mask = masks[i]
            ...
            if len(cur_points) >= self.all_points_num:
                cur_idxs = fast_sample(len(cur_points), self.all_points_num)
                cur_points = cur_points[cur_idxs]
            ...
            if include_rgb:
                points[i] = torch.concat([cur_points, colors], axis=1)
            else:
                points[i] = cur_points
        if with_all:
            return points, points_all, idxs, masks
        return points, idxs, masks

    def to_xyz_maps(self, depths, vanilla=False):
        # downsample
        ...
        cur_xs = self.points_x_downscale.cuda()[None]
        cur_ys = self.points_y_downscale.cuda()[None]
        if not vanilla:
            cur_xs *= cur_zs
            cur_ys *= cur_zs
